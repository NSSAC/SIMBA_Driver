...
class Module:
    ...
    def start(self, currentDirectory, startTick, startTime):
        self.config = str(
            currentDirectory.joinpath("module_{}.json".format(self.index))
        )
        self.status = str(
            currentDirectory.joinpath("status_{}.json".format(self.index))
        )
        ...
        if self.moduleData is not None:
    ...
    def end(self, currentDirectory, endTick, endTime):
        ...
        success = self._end(
            currentDirectory, self.__lastRunTick, self.__lastRunTime, endTick, endTime
        )
    ...
    @abstractmethod
    def _end(
        self, currentDirectory, lastRunTick, lastRunTime, endTick, endTime
    ) -> bool:
        return False
